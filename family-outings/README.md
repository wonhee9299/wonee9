# 가성비 놀거리 가이드북 모음

하루 종일 놀아도 돈이 별로 안 드는 가족 나들이 계획을 모아두는 곳.

## 기준 프로필

| 항목 | 값 |
|---|---|
| 가족 | 아빠 · 엄마 · **첫째 8세(130cm)** · **둘째 3세 후반(101cm)** |
| 기준지 | **하남 미사** |
| **차** | **전기차** — 주차·충전 판단에 영향(급속 1시간 / 완속 14시간 규정, 충전기 위치) |

> 차 정보는 2026-09-21에 추가했다. 그전 문서들은 차종을 모른 채 주차를 계획했다 — 경위는 [AUDIT ⑱](./AUDIT-2026-09-07.md).
새 지역을 조사할 때마다 `TEMPLATE.md`를 복사해서 `YYYY-MM-DD-지역명.md` 파일로 저장한다.

> **2026-09-07 감사 완료** — [감사 결과](./AUDIT-2026-09-07.md) · [1차 자료](./DATA-seoulland.md) · [다시 쓴 가이드](./seoulland-guide.md)
> 마스터 바이블의 법칙 3 폐기, 루나패스 서술 정정, 유료 체험시설 누락 등이 확인되었습니다.

## 운영 원칙

- **출처 등급을 붙인다.** A=공식 확인 / B=2차 자료 교차확인 / C=1회 관찰(표본 1 명시) / D=미확인.
  **C는 일반 가이드 본문에 넣지 않는다.** 1회 관찰을 법칙으로 승격시키지 않는다.
- **"확인 필요"는 정말 확인 불가능할 때만 쓴다.** 조사하면 알 수 있는 것을 독자에게 떠넘기지 않는다.
- **반례가 나오면 법칙을 버린다.** 조건을 덧붙여 살리지 않는다.

- **성공 기준은 많이 보기보다 정확히 쓰고 나오는 것.** 장소를 욱여넣지 않는다. 지치기 전에 끝낸다.
- **정보는 반드시 검증한다.** 운영시간·요금·연령 제한은 공식 홈페이지나 최근 방문 후기로 교차 확인하고, 출처 링크를 남긴다. 확실하지 않은 건 "현장 확인 필요"라고 표시하지 추측해서 채우지 않는다.
- **전화 확인이 필요한 항목은 체크리스트에 반드시 넣는다.** (연령 제한, 예약 필요 여부, 휴무일 등)
- **비용은 항목별로 쪼개서 총액을 미리 예상한다.** 놀라는 지출이 없어야 "가성비"다.
- **예약제는 어트랙션·공연을 따로 확인한다.** 무료인지 유료인지, 예약 오픈이 언제인지까지. 블로그·뉴스 정보는 1년만 지나도 무효이므로 방문 1~2일 전 전화로 확정한다.
- **물놀이 시설과 돗자리 가능 구역은 따로 확인한다.** 준비물과 휴식 거점이 통째로 바뀐다.
- **아이 인원·나이가 바뀌면 코스도 바뀐다.** 매번 참가자 구성을 기록해서 나중에 비교할 수 있게 한다.

## 가이드 목록

| 날짜 | 지역 | 참가자 | 코스 | 예상 총비용 | 상태 |
|---|---|---|---|---|---|
| 2026-08-22 | [동두천](./2026-08-22-dongducheon.md) | 아빠 + 첫째 8세 + 둘째 3세 후반 | 꿈이담 키즈헬스케어센터 → 아리랑갈비 → 경기북부어린이박물관 → 자유수호평화박물관 | 44,000~62,000원 | 완료 |
| 2026-09-05 | [서울랜드](./2026-09-05-seoulland.md) · [PDF 마스터북](./assets/seoulland_family_masterbook_20260905.pdf) · [지도 동선도](./assets/seoulland_map_route_20260905.jpg) · [포켓카드](./assets/seoulland_pocket_card_20260905.png) | 아빠+첫째(130cm 코스터팀) / 엄마+둘째(101cm 키즈존팀) | 동문 오픈런 → 코스터/키즈존 분산 → 12:00 브루미즈동산 도시락 → 오후 공용 → 16:00 출차 | 약 85,000원 (자유이용권 8만 + 음료) | **완료** (방문 후 메모 있음) |

## 공유용 가이드 (외부에 보내는 것)

- **[서울랜드 준비 자료](./seoulland-guide.md)** — 키·날씨·공식 코스·할인 네 축. **개인 경험 없이 공식 자료만.**
  [웹 링크](https://claude.ai/code/artifact/ed4f0a74-8cf4-4ec0-8027-476cecc5fdc7) · [PDF](./assets/seoulland_guide.pdf) · [지도](./assets/seoulland_guide_map.jpg) · [포켓카드](./assets/seoulland_guide_pocket.png)
  - 어트랙션 50종(존·보호자 동반·별도 요금) · 날씨별 운휴 공식 표 · 공식 추천코스 6종 · 진행 중 할인 17종
  - 원자료: **[DATA-seoulland.md](./DATA-seoulland.md)** · 감사 기록: [AUDIT-2026-09-07.md](./AUDIT-2026-09-07.md)

## 참고 목록 (날짜 없는 자료)

- **[놀이공원 마스터 바이블](./theme-park-bible.md)** — 서울랜드 실패에서 뽑은 5가지 법칙, 방문 전 체크리스트, 4개 파크 비교표
  - [키로 고르는 놀이공원 (한 장 비교)](https://claude.ai/code/artifact/71639e43-1c18-493c-becf-c322670de4ff) · [D-1 전화 스크립트](./call-script.md)
  - **[에버랜드 평일 마스터북](./park-everland.md)** — 풀세트: [PDF 마스터북](./assets/everland_family_masterbook_weekday.pdf) · [공식 가이드맵 동선](./assets/everland_map_route_weekday.jpg) · [포켓카드](./assets/everland_pocket_card_weekday.png) · [웹 마스터북](https://claude.ai/code/artifact/036c0b84-d712-4b8a-9b93-786a147eb1fe)
  - [롯데월드 계획서](./park-lotteworld.md) · [경주월드 계획서](./park-gyeongjuworld.md)
- [서울랜드 2회차 공략 — 아래쪽 구역 & 공연](./seoulland-tips-v2.md) — 주말 공연 시간표 실측, 3세 부적합 공연, 모험의나라·삼천리동산·백일홍가든
- **[가을 주말 빈집털이 나들이 — 하남 기준](./fall-weekend-empty.md)** (10판) — 정원제·예약제로 인원 상한이 걸린 곳만 골라 순위화. **미사역 기준 실측 도로거리**, 4인 비용, 도시락·돗자리·텐트 규정, **한강공원 과태료표**, **모래놀이터 14곳**(하남 나무고아원 포함, [PDF 9쪽·사진 12장](./assets/sand_playground_guide.pdf))(강동 유아숲체험원 4곳·광나루·보라매 등), 뺀 곳과 근거, **재조사 로그(내 오류 3건 정정)** · [모래놀이터 가이드 PDF](./assets/sand_playground_guide.pdf)(4쪽, 광나루 사진 포함)
- **[코엑스 아쿠아리움 — 주차부터 짜는 계획](./coex-aquarium.md)** (8판) — **코엑스 북문 GATE → B2 블루존 52G~60G**(→B1 한 층), 4시간 **4,800원** 할인 등록법, 옥상 주차장 425면 실측 계수, **전기차 급속 1시간 규정** 반영, GBB·VR 1회 1개, 인어·물범·가오리 **공연 3종** 기준 A/B/C/C+ 4안 + [작전카드 PDF](./assets/coex_aquarium_plan.pdf)
- [하남 기준 가성비 캠핑장 목록](./camping-near-hanam.md) — 당일치기/1박, 거리순, 4인 1박 예상 요금, 예약 오픈일, **텐트 설치형(장비 없이 가는 곳) 별도 표**
- **[레고랜드 코리아 완전 가이드](./park-legoland.md)** — 어트랙션 40개 전수 신장·나이 조건, 체험형·놀이터 총정리, 주차장 전략(도보 vs 셔틀), 동선 지도 검증, **주말/평일 스케줄 전략 2종**(실방문 후기 기반), 9/23 실제 방문 예시 일정, 주의점 체크리스트 + [지도 포함 작전카드 PDF](./assets/legoland_20260923_plan.pdf) · [문앞→문앞 PPT](./assets/legoland_20260923_plan.pptx)

## 새 가이드 만드는 법

1. `TEMPLATE.md`를 복사해 `family-outings/YYYY-MM-DD-지역명.md`로 저장
2. 장소별 운영시간·요금·연령 제한을 실제 출처(공식 사이트, 최근 후기)로 확인하며 채움
3. 스케줄·동선·비용·체크리스트까지 다 채우면 위 목록 표에 한 줄 추가
4. 방문 후 실제로 달랐던 점이 있으면 "방문 후 메모" 섹션에 남겨서 다음 계획에 반영
