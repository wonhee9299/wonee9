...
시놀로지 NAS를 서버 인프라로 쓰기 위한 저장소입니다. 두 부분으로 나뉩니다.

| 폴더 | 무엇 | 어디서 실행 |
|------|------|-------------|
| **[`infra/`](infra/)** | NAS를 서버로 만드는 도커 스택 — 진입점(Caddy), DB, 모니터링, 백업 | NAS 위에서 |
| **`nascloud/`** (아래) | 작업 파일을 NAS에 저장하고 꺼내오는 CLI | 내 PC에서 |

인프라를 구축하려면 → **[infra/README.md](infra/README.md)**
